	...
	$a->tr(
		...
		$a->td(
			$a->table(
				$a->tr(
					$a->td($L['memcache_lib'].':').
					$a->td($L[memcache()], true, '', memcache() ? 'green' : 'red')
				).
				$a->tr(
					$a->td($L['memcached_lib'].':').
					$a->td($L[memcached()], true, '', memcached() ? 'green' : 'red')
				).
				$a->tr(
					$a->td($L['mcrypt'].':').
				...
				$a->tr(
					$a->td($L['zlib'].':').
					$a->td($L[zlib()], true, '', zlib() ? 'green' : 'red')
		...
		$a->td(
			$a->table(
				$a->tr(
					...
					$a->td($L[ini_get('file_uploads')], true, '', ini_get('file_uploads') ? 'green' : 'red')
				).
				$a->tr(
				...
				$a->tr(
					$a->td('mod_rewrite:').
					$a->td(
						function_exists('apache_get_modules') && in_array('mod_rewrite',apache_get_modules()) ? $L['on'] : $L['off'],
						true,
						'',
				...
				$a->tr(
					...
					$a->td($L[get_magic_quotes_gpc()], true, '', !get_magic_quotes_gpc() ? 'green' : 'red')
				).
				$a->tr(
					$a->td($L['allow_url_fopen'].':').
					$a->td($L[ini_get('allow_url_fopen')], true, '', ini_get('allow_url_fopen') ? 'green' : 'red')
				).
				$a->tr(
					$a->td($L['display_errors'].':').
					$a->td($L[display_errors()], true, '', display_errors() ? 'green' : 'red')
				).
				$a->tr(
					$a->td('register_globals:').
					$a->td($L[register_globals()], true, '', !register_globals() ? 'green' : 'red')
				), false, true, '', 'left_table'
			)
		)
	), '', false, '', 'admin_table'
