		...
		$a->tr(
			$a->td(
				...
				$a->input(
					...
					'form_element'
				), true, ' colspan="2" style="'.($Config->core['zlib_compression'] || zlib_autocompression() ? '' : 'display: none; ').'text-align: center;"', '', 'zlib_compression'
			)
		).
		$a->tr(
		...
		$a->tr(
			$a->td(
				$a->button(
					...
					'',
					true,
					' onMouseDown="javascript: admin_cache(\'#clean_settings_cache > div\', \''.$a->action.'/settings/nointerface\');"'
				)
			).
			$a->td(
				$a->button(
					...
					'',
					true,
					' onMouseDown="javascript: admin_cache(\'#clean_scripts_styles_cache > div\', \''.$a->action.'/jscss/nointerface\');"'
				)
			)
		).
		$a->tr(
