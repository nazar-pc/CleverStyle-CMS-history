		...
		if ($errfile && $errline) {
			...
			switch ($errno) {
				...
				case E_ERROR:
					...
					$Page->content("<p><b>$L[fatal] #$errno:</b> $errstr $L[in_line] $errline $L[of_file] "
				...
				case E_WARNING:
					...
					$Page->content("<b>$L[error] #$errno:</b> $errstr $L[in_line] $errline $L[of_file] "
								."$errfile, PHP ".PHP_VERSION." (".PHP_OS.")<br>"
								."$L[report_to_admin]<br>\n"
								.(is_object($Config) ?
									($Config->core['admin_mail'] ? $L['admin_mail'].': <a href="mailto:'.$Config->core['admin_mail']."\">".$Config->core['admin_mail']."</a><br>\n" : '')
				...
				case E_NOTICE:
					...
					$Page->content("<b>$L[warning] #$errno:</b> $errstr $L[in_line] $errline $L[of_file] "
								."$errfile, PHP ".PHP_VERSION." (".PHP_OS.")<br>");
				break;
